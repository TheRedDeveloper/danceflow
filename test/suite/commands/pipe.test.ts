...
suite("./test/suite/commands/pipe.md", function () {
  ...
  test("1 > pipe-replace-with-regexp", async function () {
    ...
    await executeCommand("danceflow.selections.pipe.replace", { expression: String.raw`/\s/-/g` });
  ...
  test("1 > pipe-replace-with-regexp-newline", async function () {
    ...
    // Perform all operations.
    await executeCommand("danceflow.selections.pipe.replace", { expression: String.raw`/\s/\n/g` });
  ...
  test("1 > pipe-replace-with-js", async function () {
    ...
    // Perform all operations.
    await executeCommand("danceflow.selections.pipe.replace", { expression: String.raw`$.replace(/\s/g, "-")` });
